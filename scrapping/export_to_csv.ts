import { Event } from "./scrap_bilety24";
...
async function exportEvents() {
    ...
    fs.readFile("exports/events.json", (async (err, data) => {
...
export async function exportToCsvAndDownloadImages(eventsByCategory: { [id: string]: Event[] }) {
    ...
    fs.writeFile("exports/products.csv", csv, (err => {
        if (err) throw err
    }));
}
...
/*type PrestashopProduct = {
    ...
    name: string,
    categories: string,
    priceTaxExcluded: number, // netto
    taxRulesID: number,
    wholesalePrice: number, // brutto
    onSale: 0 | 1,
